...
letter in the plain text is replaced by a letter some fixed number of positions down the alphabet. For
example, with a shift of 3, A would be replaced by D, B would become E, and so on. The
method is named after Julius Caesar, who used it to communicate with his generals. ROT-13 ("rotate
by 13 places") is a widely used example of a Caesar cipher where the shift is 13. In Python, the key
...
you will be able to read the following secret message: Pnrfne pvcure? V zhpu cersre Pnrfne fnynq!
...
str = ''
user = input("please sentence to decode")
for i in user:
    if i == " ":
        str += i
    else:
